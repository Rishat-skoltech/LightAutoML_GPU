...
from ...ml_algo.base_gpu import TabularMLAlgo_gpu, TabularDatasetGpu
...
from ...reader.utils_gpu import set_sklearn_folds_gpu
...
class NestedTabularMLAlgo_gpu(TabularMLAlgo_gpu, OptunaTunableMixin, ImportanceEstimator):
    """
    ...
    def __init__(self, ml_algo: TabularMLAlgo_gpu, tuner: Optional[ParamsTuner] = None, refit_tuner: bool = False,
    ...
    def fit_predict(self, train_valid_iterator: TrainValidIterator):
    ...
    def fit_predict_single_fold(self, train: TabularDatasetGpu, valid: TabularDatasetGpu) -> Tuple[Any, np.ndarray]:
        ...
        # TODO: rewrite
        train.folds = set_sklearn_folds_gpu(train.task, train.target, self.nested_cv, random_state=42, group=train.group)
    ...
    def predict_single_fold(self, model: Any, dataset: TabularDatasetGpu) -> np.ndarray:
    ...
    def get_features_score(self) -> Series:
        print("DOING THE CUDF CONCAT WHICH WAS NEVER TESTED")
        #maybe leave pd.concat
        scores = cudf.concat([x.get_features_score() for x in self.models], axis=1).mean(axis=1)
