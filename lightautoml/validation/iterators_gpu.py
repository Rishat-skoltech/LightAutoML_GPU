...
CupyOrCudf = Union[CupyDataset, CudfDataset, DaskCudfDataset]
...
class FoldsIterator(TrainValidIterator):
    ...
    def __init__(self, train: CupyOrCudf, n_folds: Optional[int] = None):
        """Creates iterator.
    ...
    def __next__(self) -> Tuple[cp.ndarray, CupyOrCudf, CupyOrCudf]:
        ...
        self._curr_idx += 1
        return val_idx, cast(CupyOrCudf, train), cast(CupyOrCudf, valid)

    def get_validation_data(self) -> CupyOrCudf:
...
@record_history(enabled=False)
def get_cupy_iterator(train: CupyOrCudf, valid: Optional[CupyOrCudf] = None,
...
class TimeSeriesIterator:
    ...
    def split_by_dates(datetime_col, splitter):
        ...
        splitter = cp.sort(splitter)
        folds = cp.searchsorted(splitter, datetime_col)
    ...
    def split_by_parts(datetime_col, n_splits: int):
        ...
        idx = cp.arange(datetime_col.shape[0])
        order = cp.argsort(datetime_col)
        sorted_idx = idx[order]
        folds = cp.concatenate([[n] * x.shape[0] for (n, x) in
                                enumerate(cp.array_split(sorted_idx, n_splits))], axis=0)
    ...
    def __init__(self, datetime_col, n_splits: Optional[int] = 5,
        ...
        uniques = cp.unique(folds)
        assert (uniques == cp.arange(uniques.shape[0])).all(), 'Fold splits is incorrect'
    ...
    def __getitem__(self, item) -> Tuple[cp.ndarray, cp.ndarray]:
        ...
        idx = cp.arange(self.folds.shape[0])
