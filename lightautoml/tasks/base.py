...
from .utils_gpu import infer_gib_gpu, infer_gib_multiclass_gpu,\
                       infer_gib_mgpu, infer_gib_multiclass_mgpu
...
class Task:
    ...
    def __init__(self, name: str, loss: Optional[Union[dict, str]] = None, loss_params: Optional[Dict] = None,
        ...
        if device is None:
            device = 'cpu'
        assert device in ['cpu', 'gpu', 'mgpu'], 'The device must be either `cpu`, `gpu` or `mgpu`!'
        ...
        if type(metric) is str:
            ...
            if self.device == 'gpu':
                ...
                metric_func = _valid_str_metric_names_mgpu[self.name][metric]
        ...
        if greater_is_better is None:
            if self.device == 'cpu':
                ...
                infer_gib_fn = infer_gib_multiclass_mgpu if name == 'multiclass' else infer_gib_mgpu
            else:
    ...
    def get_dataset_metric(self) -> LAMLMetric:
        ...
        if self.device == 'cpu':
            ...
            dataset_metric = DaskmlMetric(self.metric_func, name=self.metric_name,
                                        one_dim=one_dim, greater_is_better=self.greater_is_better)
