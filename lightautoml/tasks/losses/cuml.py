...
class CUMLLoss(Loss):
    ...
    def __init__(self, loss: str, loss_params: Optional[Dict] = None, fw_func: Optional[Callable] = None,
        ...
        #assert devices here, don't forget
        self.device = device
    ...
    def metric_wrapper(self, metric_func: Callable, greater_is_better: Optional[bool],
        ...
        if greater_is_better is None:
            ...
            assert self.device in ['cpu', 'gpu', 'mgpu'], "Device should be either 'cpu', 'gpu' or 'mgpu'"

            if self.device == 'mgpu':
                greater_is_better = infer_gib_mgpu(metric_func)
            else:
