...
from ..common_metric_gpu import _valid_str_multiclass_metric_names_gpu,\
                                _valid_str_multiclass_metric_names_mgpu
from ..utils_gpu import infer_gib_gpu, infer_gib_mgpu
...
_xgb_multiclass_metrics_dict_mgpu = {
    'auc': _valid_str_multiclass_metric_names_mgpu['auc'],
    #'auc_mu': _valid_str_multiclass_metric_names_mgpu['auc_mu'],
...
_xgb_metrics_dict_mgpu = {
    'binary': _xgb_binary_metrics_dict,
    'reg': _xgb_reg_metrics_dict,
    'multiclass': _xgb_multiclass_metrics_dict_mgpu
}
...
class XGBLoss_mgpu(Loss):
    ...
    def set_callback_metric(self, metric: Union[str, Callable], greater_is_better: Optional[bool] = None,
        ...
        if type(metric) is str:
            ...
            _metric_dict = _xgb_metrics_dict_mgpu[task_name]
            _metric = _metric_dict.get(metric)
            if type(_metric) is str:
                self.metric_name = _metric
                self.feval = None
