...
class TabularMLAlgo(MLAlgo):
    ...
    def _set_prediction(self, dataset: CupyDataset, preds_arr: cp.ndarray) -> CupyDataset:
        ...
        dataset.set_data(preds_arr, prefix, NumericRole(cp.float32, force_input=True, prob=prob))

        return dataset

    def fit_predict_single_fold(self, train: TabularDataset, valid: TabularDataset) -> Tuple[Any, cp.ndarray]:
    ...
    def predict_single_fold(self, model: Any, dataset: TabularDataset) -> cp.ndarray:
    ...
    def predict(self, dataset: TabularDataset) -> CupyDataset:
