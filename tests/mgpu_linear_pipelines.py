...
from lightautoml.ml_algo.linear_gpu import LinearL1CD_mgpu
...
def test_pipeline():
    ...
    reg_0 = LinearL1CD_mgpu(client, timer=timer_reg)
    ...
    task = Task('reg', metric = 'mse', device='mgpu') 
    
    reader = DaskCudfReader(task = task, samples = None, max_nan_rate = 1,
                              max_constant_rate = 1, advanced_roles = False,
