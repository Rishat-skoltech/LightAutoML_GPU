...
def test_multiclass():
    ...
    daskcudf_reader = DaskCudfReader(task, device_num=0, cv=5, random_state=42, n_jobs=1, compute=True, advanced_roles=True)

    hybrid_reader = HybridReader(task, num_cpu_readers=2, num_gpu_readers=1, gpu_ratio=0.5, output='gpu', cv=5, random_state=42)
